====================================
Rosicast's documentation
====================================

I have no intention to break the copyright. I just want to share a great contents from others and organize those to make easy to use. if there are issue, please contact me via email or github. Thanks.

Links

* `Petar Veličković <https://petar-v.com>`_
* `Geometric Deep Learning <https://geometricdeeplearning.com/>`_

Libraries

* `Pyg Docs <https://pytorch-geometric.readthedocs.io/en/latest/>`_
* `Jraph <https://jraph.readthedocs.io/en/latest/#>`_
* `Jax <https://jax.readthedocs.io/en/latest/index.html>`_
* `Haiku <https://dm-haiku.readthedocs.io/en/latest/index.html>`_
* `Flax <https://flax.readthedocs.io/en/latest/>`_
* `Optax <https://optax.readthedocs.io/en/latest/>`_

Datasets

* `Open Graph Benchmark <https://ogb.stanford.edu/#>`_


.. toctree::
    :caption: Notation
    :maxdepth: 1

    math_notation

.. toctree::
    :caption: Category_theory
    :maxdepth: 1
    
    ipynbs/Category_theory/Definitions

.. toctree::
    :caption: GDL_book
    :maxdepth: 1
    :glob:

    ipynbs/GDL/book/*

.. toctree::
    :caption: Papers
    :maxdepth: 1
    :glob:

    ipynbs/Papers/*

.. toctree::
    :caption: AMMI_ex
    :maxdepth: 1
    :glob:

    ipynbs/AMMI_ex/*

.. toctree::
    :caption: Notes
    :maxdepth: 1
    :glob:

    ipynbs/Notes/*


Indices and tables
==================

* :ref:`genindex`
* :ref:`modindex`
* :ref:`search`